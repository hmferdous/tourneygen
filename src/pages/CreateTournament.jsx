...
<select value={format} onChange={e => setFormat(e.target.value)} className="w-full mb-4 p-2 border rounded">
...
<input type="number" min={3} max={16} value={teamCount} onChange={e => setTeamCount(Number(e.target.value))} className="w-24 mb-4 p-2 border rounded" />


{format === 'group_knockout' && (
<div className="mb-4">
<label>Number of Groups</label>
<input type="number" min={2} max={Math.floor(teamCount / 3)} value={groupCount} onChange={e => setGroupCount(Number(e.target.value))} className="w-24 p-2 border rounded" />
<p className="text-sm text-gray-500">Groups will be distributed evenly. Minimum 3 teams per group recommended.</p>
</div>
)}
...
<input key={i} defaultValue={t} onChange={e => {
const copy = [...teams]
...
<button onClick={handleGenerate} className="px-4 py-2 bg-green-600 text-white rounded">Generate Fixtures</button>
</div>
)
}